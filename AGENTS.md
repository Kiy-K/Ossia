...
Ossia — a portable, model-agnostic support agent built on LangChain Deep Agents. The unified HTTP API at `/v1/*` is the only runtime entry point; CLI scripts and the notebook are thin HTTP clients. Spec-driven: `specs/openapi.checked.json` is the pinned contract, `tests/test_openapi_drift.py` fails the suite on drift.

Architecture and intent: `README.md` (overview), `specs/SPEC.md` (narrative spec), `docs/adr/0001..0005.md` (the five design decisions). Read those before changing behavior.
...
# Start the server
OSSIA_API_KEY=dev .venv/bin/python -m uvicorn ossia.api:app --host 127.0.0.1 --port 8000
```
...
```
src/ossia/           # Library: agent, memory, tools, mcp_tools, middleware,
                     # schemas, audit, eval, cli_helper, api
...
docs/adr/            # 0001..0005 — design decisions
nebius/              # deploy.sh, docker/, endpoints/*.yaml, jobs/*.yaml
notebooks/demo.ipynb # HTTP client via httpx
```

The real entrypoints:
- Server: `ossia.api:app` (FastAPI). Lifespan builds the agent via `build_agent_async`.
- Audit: `ossia.audit.run_audit()` returns `AuditReport`.
- Eval: `ossia.eval.run_eval()` returns `EvalReport`.
- CLI helpers: `ossia.cli_helper` (subprocess, health-check, require_api_key).
- MCP: `ossia.mcp_tools.MCPToolkit` — worker-per-task, graceful degradation.
...
- The v3 streaming protocol is marked `@beta` upstream. The `ossia.api.chat_stream` handler adapts the typed projections to our wire contract (`kind` + per-kind `data`). If upstream changes projection attribute names, only the adapter needs to update; clients are insulated by the wire contract.
- **Memory surfaces** (see ADR-0007):
  - **Long-term / semantic** lives in the LangGraph store at
    ...
    written by `ossia.memory.seed_memory` on first boot and is
  ...
  - **Episodic / per-thread recall** is the `recall_thread_turns`
    tool from `ossia.episodic`. It calls
...
- Don't bypass the FastAPI server in new CLIs. Use `httpx` against the running server (or spin one up via `ossia.cli_helper.run_server_subprocess`).
- Don't import `ossia.agent` from CLIs or the notebook. Drive the agent through the HTTP API.
- Don't change the audit/eval logic in `scripts/` — that lives in `src/ossia/audit.py` and `src/ossia/eval.py`. The CLIs are presentational.
